...
const openedMixin = (theme) => ({
  ...
  [theme.breakpoints.down("sm")]: {
    height: "fit-content",
    margin: "114px 1px",
    borderRight: "solid 1px var(--color-bg)",
    borderBottom: "solid 1px var(--color-bg)",
    paddingBottom: "16px",
  },
});
...
export default function DrawerXDashTable({ onLogout, children }) {
  ...
  useEffect(() => {
    const menuDiv = document.querySelector(".dashmenuSpan");
    const menuToggler = document.querySelector(".more-button");

    // Immediately check and set the initial visibility based on window width
    if (window.innerWidth < 768 && menuDiv) {
    ...
    if (menuDiv && menuToggler) {
      const handleClick = () => {
        menuDiv.style.display = menuDiv.style.display === "none" ? "block" : "none";
      };

      menuToggler.addEventListener("click", handleClick);

      window.addEventListener("resize", resize);

      // Cleanup function to remove event listeners
      return () => {
        menuToggler.removeEventListener("click", handleClick);
  ...
  return (
    <Box sx={{ display: "flex" }} className={Styles.EverythingWrapper}>
      <button className="more-button" aria-label="Menu Button">
